...
class CovWeights:
    ...
    spw              : list
        List of spectral windows for which to calculate the weights. By default, is None
        and the code simply iterates sequentially over all spectral windows present in
        the MS file. If spectral window indices are informative, strongly recommended
        only asking for the relevant windows.
    """
    ...
    def __init__(self, MSName, dt=0, nt=0, dfreq=0, nchan=0, SaveDataProducts=True, \
                 ...
                 useWS=False,spw=None,spwnorm=False):
        """
        A class used to generate interferometric quality-based weighting scheme values.
        Standard use is to instantiate the class, then FindWeights, SaveWeights, close,
        and CreateDiagnostics.
        ...
        ### spwlist addition from here
        self.spwnorm          = spwnorm
        self.spwcol           = self.ms.getcol("DATA_DESC_ID")
        if spw is None:
            # spectral window indices are contained in DATA_DESC_ID
            self.spwlist      = np.unique(self.spwcol)
        else:
            self.spwlist      = spw
        self.nspw             = len(self.spwlist)
        if self.verbose:
            print("Calculating weights for spectral windows : ",self.spwlist)
        ### spwlist addition ends here
        ...
        self.CoeffArray       = np.zeros((self.nspw,self.nAnt, self.nt, self.nChan))
    ...
    def FindWeights(self):
        ...
        print("In debug, contact etienne.bonnassieux@u-bordeaux.fr to get a fix")
        exit()
    ...
    def FindIrregularWeights(self):
        ...
        for ispw, spw in enumerate(self.spwlist):
            if self.verbose:
                print("Calculating for SPW %i"%spw)
            ...
            for t_val in self.tvals:
                ...
                for ant in self.ant1:
                    ...
                    for chan_i in chanrange:
                        ...
                        weights    = weightvals[:,chanmin:(chan_i+self.dchan),:].astype(bool)
                        if np.sum(weights) > 0:
                            self.CoeffArray[ispw, ant, t_i, chan_i] = np.average( np.real( vals * vals.conj() ), \
                                                                            weights = weights)
                        else:
                            # if flagged/absent, set var estimates to 0
                            self.CoeffArray[ispw, ant, t_i, chan_i] = 0
                if self.verbose:
                    PrintProgress(t_i,self.nt)
                t_i = t_i + 1
        ### clean up coeffarray before writing.
        # remove nans
        self.CoeffArray[np.isnan(self.CoeffArray)]=np.inf
        # normalise  per spw if requested
        if self.spwnorm:
            if self.verbose:
                print("Normalise CoeffArray per spectral window")
            for ispw in range(self.nspw):
                # check that full spectral window is not flagged
                spwcoeffs = self.CoeffArray[ispw,:,:,:]
                if np.sum(spwcoeffs)!=0:
                    self.CoeffArray[ispw,:,:,:] = self.CoeffArray[ispw,:,:,:] / \
                        np.average( self.CoeffArray[ispw,:,:,:], weights = self.CoeffArray[ispw,:,:,:].astype(bool))
                # if also requested, normalise per antenna within each spw
                if self.antnorm:
                    if self.verbose:
                        print("Normalise CoeffArray per antenna within each spectral window")
                    for j in range(self.nAnt):
                        antcoeffs = self.CoeffArray[ispw,j,:,:]
                        # check that the full antenna is not flagged
                        if np.sum(antcoeffs)!=0:
                            self.CoeffArray[ispw,j,:,:] = np.average(antcoeffs, weights=antcoeffs.astype(bool))
        # normalise per antenna and not per spw if requested
        if self.antnorm==True and self.spwnorm==False:
            if self.verbose:
                print("Normalise per antenna including all spectral windows")
            for iAnt in range(self.nAnt):
                ...
                if np.sum(antcoeffs)!=0:
                    ...
                    self.CoeffArray = self.CoeffArray /    \
                        np.average( self.CoeffArray, weights = self.CoeffArray.astype(bool))
        ### if saving data products, create diagnostic directory if not yet created
        if self.SaveDataProducts:
            if not os.path.exists(self.DiagDir):
                os.makedirs(self.DiagDir)
            coeffFilename = self.DiagDir+"CoeffArray."+self.basename+".npy"
...
def readArguments():
    ...
    parser.add_argument("--modelcol",  type=str,   help="Name of the model data column used to create residual data. "+\
    ...
    parser.add_argument("--slidingsolve", help="Instead of using bins and averaging across dt/dnu, slide "+\
                        ...
                        required=False,action="store_true")
    parser.add_argument("--spws",help="Spectral window(s) for which to calculate weights. Default iterates over"+
                        "all spectral windows in the MS file. Specify as : --spw [2,3,4,5] ", required=False,default=None)
    parser.add_argument("--NormPerSPW", help="Normalise gains per spectral window to avoid relative suppression of specific spws", \
			required=False,action="store_true")
